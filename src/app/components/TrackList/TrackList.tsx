...
import { GuidelinesProvider, Guidelines, useGuidelines } from "@/app/features/guidelines/guidelines-implementation";
...
function TrackListContent() {
 ...
 const renderClipHeader = useCallback((clip: ClipInterface, trackId: number) => {
   return (
     <DraggableClipHeader
       clip={clip}
       trackId={trackId}
 ...
 }, [updatePosition, startTracking, stopTracking]);
 ...
 const handleEmptySpaceDragOver = (e: React.DragEvent) => {
   ...
   const trackElements = containerRef.current?.querySelectorAll('[class*="trackContainer"]');
 ...
 const handleEmptySpaceDrop = (e: React.DragEvent) => {
   ...
   if (isBelowAllTracks && isDraggingOverEmpty) {
     ...
     console.log('Drop detected with data:', { clipId, sourceTrackId, offsetX });
     
     if (clipId && sourceTrackId) {
       // Calculate position where the clip was dropped
       const rawPosition = Math.max(0, e.clientX - containerRect.left - offsetX);
 ...
 return (
   <div
     ref={containerRef}
     className={`${styles.trackListContainer} ${isDraggingOverEmpty ? styles.dragOverEmpty : ''}`}
     style={{ position: "relative", minHeight: "300px", paddingBottom: "100px" }}
